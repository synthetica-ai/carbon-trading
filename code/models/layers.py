...
class GraphAttentionEncoder(tf.keras.layers.Layer):
    ...
    def call(self, x, mask=None):
        ...
        x = tf.concat((self.init_embed_depot(x[0])[:, None, :],  # (batch_size, 2) --> (batch_size, 1, 2)
                       self.init_embed(tf.concat((x[1], x[2][:, :, None],
                                                x[3][:, :, None],x[4][:, :, None]), axis=-1))  # (batch_size, n_nodes-1, 2) + (batch_size, n_nodes-1)
...
class GraphAttentionDecoder(tf.keras.layers.Layer): # TODO bug is here
    ...
    def get_step_context(self, state, embeddings,vehicle_id):
        ...
        re = [tf.concat(x,-1) for x in zip(all_cur_embedded_node_list,dems_list,times_list)]
    ...
    def call(self, inputs, embeddings, context_vectors, node_cap, vehicle_cap,duration_tensor,distance_tensor):
        ...
        state = self.problem(inputs, node_cap, vehicle_cap,duration_tensor,distance_tensor)
        ...
        while not state.all_finished():
            ...
            for vehicle_id in range(len(state.vehicle_list)):
                ...
                # get current mask
                mask = state.get_mask(vehicle_id)  # (batch_size, 1, n_nodes) with True/False indicating where agent can go
        ...
        return tf.stack(outputs, 1), tf.cast(tf.stack(sequences, 1), tf.float32),vehicles_used,cost,distance_cost,earliness_cost,tardiness_cost
